...
- 眼在手上的期望目标位置默认为 TCP 坐标系下 `[0, 0, 0.115] m`，即目标
  对齐夹爪中心线。腕部相机光心相对夹爪沿 X 偏移 `0.060 m`，并沿接近方向
  比 TCP 后缩约 `0.035 m`；TF 会自动处理该安装偏移，对应的相机深度仍约为
  `0.15 m`。这是预抓取对准距离而不是接触距离。
...
常用参数现也可直接在 RViz 的 `aubo_ros_control/VisualServoPanel` 中修改，无需启动
`rqt_reconfigure`。面板提供位置/姿态增益、线/角速度上限、对齐死区、目标超时和
丢失策略，点击“应用参数”后读取节点返回的实际配置。其余高级参数仍可通过已有
动态参数接口调整。在线值不会自动保存到 YAML。
