...
bool SortingTask::pickAndPlace(const aubo_perception::DetectedObject& detected)
{
  ...
  ROS_INFO("Picking %s at [%.3f, %.3f, %.3f]", category.c_str(), object_x, object_y, grasp_z);
  ...
  std::string object_model_name =
      model == grasp_model_names_.end() ? category + "_block" : model->second;
  ...
  {
    ROS_ERROR_STREAM("No place target configured for category '" << category << "'");
    return false;
  }
  double place_x = 0.0;
  double place_y = 0.0;
  ...
  if (!cartesianTo(makePose(place_x, place_y, expected_center + grasp_height_offset_ + place_clearance_), category + " place") || // 移动到目标放置位置
    ...
    return false;
  completed = cartesianTo(makePose(place_x, place_y, preplace_z), category + " retreat");
  return completed; // 移动到目标放置位置上方
}

bool SortingTask::sortingOperation()
{
  if (continuous_sorting_)
    return continuousSortingOperation();
  bool all_complete = !sort_classes_.empty();// 分拣类别列表不为空
  for (const auto& category : sort_classes_)
    all_complete = all_complete && completed_categories_.count(category) != 0;// 检查所有类别是否已经完成
  if (all_complete)
  {
    completed_categories_.clear();// 若都完成分拣了，清空已完成类别列表
    {
      std::lock_guard<std::mutex> lock(data_mutex_);
  ...
  for (std::size_t index = 0; index < sort_classes_.size(); ++index)
  {
    const std::string& category = sort_classes_[index];
    if (stop_requested_.load())
      return false;
    if (completed_categories_.count(category))
    {
      ROS_INFO("Skipping completed category '%s' in workspace '%s'", category.c_str(), workspace_id_.c_str());
      continue;
    }
    publishState(State::DETECTING, category);
    aubo_perception::DetectedObject detected;// 检测到的目标对象消息：颜色、面积、位置
    if (!waitForObject(category, ros::WallTime::now(), detected))// 获取目标对象消息
      return false;
    observation_ready_.store(false);
    publishState(State::PICKING, category);
    if (!pickAndPlace(detected))
      return false;
    completed_categories_.insert(category);// 标记该类别物体已完成抓取
    markTargetPicked(category);
    if (index + 1 < sort_classes_.size())
    {
