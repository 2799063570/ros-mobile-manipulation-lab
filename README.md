# 基于 ROS 的 AUBO 机械臂、移动机器人及复合机器人项目

本仓库是一个兼容 **ROS 1 Melodic / Noetic、Gazebo Classic 和 MoveIt** 的机器人
学习与开发项目，主要包含三部分内容：

- AUBO i5 六轴机械臂的建模、仿真、运动规划、视觉识别与抓取分拣；
- 差速移动机器人的建模、控制、SLAM、定位、导航与自主探索；
- AUBO 机械臂与移动底盘的系统集成，实现自主导航、目标识别、抓取和分类放置。

项目早期参考了 WheelTec ROS 功能包，用于学习底盘驱动、传感器、TF、SLAM 和导航
等实现方式。在此基础上，自主搭建差速移动机器人，并进一步与 AUBO i5 机械臂及
视觉系统集成，形成完整的复合移动机器人系统。
