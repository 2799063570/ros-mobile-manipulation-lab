...
void QNode::init_pubsub()
{
    ...
    image_sub=n.subscribe("/camera/rgb/image_raw/compressed",100,&QNode::imagepre_callback,this);
...
void QNode::sub_camera(QString cameratopic)
{
    ...
    image_transport::ImageTransport it_(n);
    qDebug()<<"image sub:";
    if(cameratopic.contains("compress"))image_sub=n.subscribe(cameratopic.toStdString(),100,&QNode::imagepre_callback,this);
    else image_sub=n.subscribe(cameratopic.toStdString(),100,&QNode::image_callback,this);
    ros::spinOnce();
}
...
void QNode::imagepre_callback(const sensor_msgs::CompressedImage &msg)
{
    cv_bridge::CvImagePtr cv_ptr;
    try
    {
