...
{
    //ssh
    QString strIp = QString("192.168.0.100");
...
void qt_ros_test::MainWindow::on_checkBox_key_clicked(bool checked)
{
    ...
    {
        QString strCmd = QString("roslaunch turn_on_wheeltec_robot turn_on_wheeltec_robot.launch\n");
        emit sigSend(strCmd);
        btn_disableall(true);
...
void qt_ros_test::MainWindow::slotDataArrived(QString strMsg, QString strIp, int nPort)
{
    Q_UNUSED(strIp)
    Q_UNUSED(nPort)
    ui.textEdit_cmd->append("<font color=\"#000000\">"+strMsg+"</font>");
...
void qt_ros_test::MainWindow::on_pushButton_laserfollower_clicked(bool checked)
{
    ...
    {
        QString strCmd = QString("roslaunch simple_follower laser_follower.launch\n");
        emit sigSend(strCmd);
...
void qt_ros_test::MainWindow::on_pushButton_visfollower_clicked(bool checked)
{
    ...
    {
        QString strCmd = QString("roslaunch simple_follower visual_follower.launch\n");
        emit sigSend(strCmd);
        ui.checkBox__opencamera->setChecked(true);
        ui.checkBox_key->setChecked(true);
        btn_disableall(true);
...
void qt_ros_test::MainWindow::on_pushButton_xfmic_clicked(bool checked)
{
    ...
    else
    {
        closeroslaunch();
        ui.checkBox_key->setChecked(false);
        ui.textEdit_cmd->append("-------------------------------------------正在关闭，请稍候-------------------------------------------");
    }
}
...
void qt_ros_test::MainWindow::on_pushButton_joy_control_clicked(bool checked)
{
    ...
    {
        QString strCmd = QString("roslaunch wheeltec_joy joy_control.launch\n");
        emit sigSend(strCmd);
    ...
    {
        closeroslaunch();
        ui.textEdit_cmd->append("-------------------------------------------正在关闭无线手柄控制，请稍候-------------------------------------------");
    }
}
...
void qt_ros_test::MainWindow::on_pushButton_2dmap_clicked(bool checked)
{
    ...
    {
        QString strCmd = QString("roslaunch turn_on_wheeltec_robot mapping.launch\n");
        emit sigSend(strCmd);
        ui.checkBox_key->setChecked(true);
        btn_disableall(true);
    ...
    {
        closeroslaunch();
        ui.textEdit_cmd->append("-------------------------------------------正在关闭2D建图，请稍候-------------------------------------------");
...
void qt_ros_test::MainWindow::on_pushButton_2dnav_clicked(bool checked)
{
    ...
    {
        QString strCmd = QString("roslaunch turn_on_wheeltec_robot navigation.launch\n");
        emit sigSend(strCmd);
        btn_disableall(true);
...
void qt_ros_test::MainWindow::on_pushButton_rrt_clicked(bool checked)
{
    ...
    {
        QString strCmd = QString("roslaunch turn_on_wheeltec_robot rrt_slam.launch\n");
        emit sigSend(strCmd);
        btn_disableall(true);
    ...
    {
        closeroslaunch();
        ui.textEdit_cmd->append("-------------------------------------------正在关闭自主建图，请稍候-------------------------------------------");
    }
}
...
void qt_ros_test::MainWindow::on_pushButton_moveitik_clicked(bool checked)
{
    ...
    {
        QString strCmd = QString("roslaunch arm_demo ik_demo.launch\n");
        emit sigSend(strCmd);
        btn_disableall(true);
    ...
    {
        closeroslaunch();
        ui.textEdit_cmd->append("-------------------------------------------正在关闭机械臂逆解，请稍候-------------------------------------------");
    }
}
...
void qt_ros_test::MainWindow::on_pushButton_moveitfk_clicked(bool checked)
{
    ...
    {
        QString strCmd = QString("roslaunch arm_demo fk_demo.launch\n");
        emit sigSend(strCmd);
        btn_disableall(true);
...
void qt_ros_test::MainWindow::on_pushButton_nfsrobot_clicked(bool checked)
{
    ...
    QString str_cmd = QString("echo '%1' | sudo -S mount -t nfs 192.168.0.100:/home/wheeltec/wheeltec_robot /mnt\n").arg(QString::fromStdString(user_passward));
...
void qt_ros_test::MainWindow::on_pushButton_unfsrobot_clicked(bool checked)
{
    ...
    _cmd->start("bash");
    _cmd->waitForStarted(5000);
    QString str_cmd = QString("echo '%1' | sudo -S umount -t nfs 192.168.0.100:/home/wheeltec/wheeltec_robot /mnt\n").arg(QString::fromStdString(user_passward));
	ui.textEdit_cmd->append(str_cmd);
    _cmd->write(str_cmd.toLocal8Bit());
}
...
{
    QString str_cmd=QString("cd ~/wheeltec_robot;catkin_make -DCATKIN_WHITELIST_PACKAGES=''\n");
	ui.textEdit_cmd->append(str_cmd);
    emit sigSend(str_cmd);
}
...
{
    QString str_cmd=QString("cd ~/wheeltec_robot;catkin_make -DCATKIN_WHITELIST_PACKAGES='")+ui.lineEdit_pkg->text()+"'\n";
	ui.textEdit_cmd->append(str_cmd);
    emit sigSend(str_cmd);
}

void qt_ros_test::MainWindow::on_pushButton_pkgmove_clicked(bool checked)
{
    _cmd->write("exit\n");
    _cmd->waitForFinished(10000);
    _cmd->start("bash");
    _cmd->waitForStarted(5000);
    QString ss= QString::fromStdString(ros::package::getPath("qt_ros_test"));
    QString str_cmd;
    if(ui.comboBox_pkg->currentText().contains("astrapro"))
        str_cmd = QString("cp -r ")+ss+QString("/package/")+ui.comboBox_pkg->currentText()+QString("/wheeltec_arm/* /mnt2/;echo '%1' | sudo -S chmod 777 -R /mnt2/src;").arg(QString::fromStdString(user_passward))
                         +QString("cp -r ")+ss+QString("/package/")+ui.comboBox_pkg->currentText()+QString("/wheeltec_robot/* /mnt/;echo '%1' | sudo -S chmod 777 -R /mnt/src\n").arg(QString::fromStdString(user_passward));
    else
        str_cmd = QString("cp -r ")+ss+QString("/package/")+ui.comboBox_pkg->currentText()+QString("/* /mnt/src/;echo '%1' | sudo -S chmod 777 -R /mnt/src\n").arg(QString::fromStdString(user_passward));
    _cmd->write(str_cmd.toLocal8Bit());
}
