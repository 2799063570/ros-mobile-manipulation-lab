...
/**************************************
Date: June 8, 2021
Function: The topic of joint state subscription Callback function, according to the joint state subscription through the serial port command to control the lower robot arm
功能: 关节状态话题订阅回调函数Callback，根据订阅的关节状态通过串口发指令控制下位机机械臂
***************************************/
void turn_on_robot::joint_states_Callback(const sensor_msgs::JointState arm_joint)
{
  ...
  //机械爪
  transition=0;
  transition = arm_joint.position[6]*1000; //将浮点数放大一千倍，简化传输
...
{ 
  ...
  */
  ...
  if (Receive_Data.Frame_Header == FRAME_HEADER ) //Judge the frame header //判断帧头
  {
    ...
    { 
      if (Receive_Data.rx[22] == Check_Sum(22,READ_DATA_CHECK)||(Header_Pos==(1+Tail_Pos))) //BCC check passes or two packets are interlaced //BCC校验通过或者两组数据包交错
      {
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<std::string>("JointStates_topic", JointStates_topic, "Arm_2_JointStates");
  ...
  joint_state_Sub = n.subscribe(JointStates_topic, 100, &turn_on_robot::joint_states_Callback, this);
...
turn_on_robot::~turn_on_robot()
{
  ...
  Send_Data.tx[7] = 0;    
  Send_Data.tx[9]=Check_Sum(9,SEND_DATA_CHECK); //Check the bits for the Check_Sum function //校验位，规则参见Check_Sum函数
  Send_Data.tx[10]=FRAME_TAIL_ARM; 
  try
  {
    Stm32_Serial.write(Send_Data.tx,sizeof (Send_Data.tx)); //Send data to the serial port //向串口发数据  
  }
