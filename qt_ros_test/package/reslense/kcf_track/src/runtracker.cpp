...
public:
  ...
  {
    cv_bridge::CvImagePtr cv_ptr;
    float total_distance = 0, distance = 0, max_distance=0, min_distance=5000;
    ...
    if(enable_get_depth)
    {
      for(int height=result.y; height<result.y+2*result.height/3; height++)
      {
        for(int width=result.x; width<result.x+2*result.width/3; width++)
        {
          if(depthimage.at<u_int16_t>(height, width)!=0)
          {
            total_distance = total_distance + depthimage.at<u_int16_t>(height, width);
            num_depth_points++;
            if(depthimage.at<u_int16_t>(height, width)>max_distance)
            {
              max_distance = depthimage.at<u_int16_t>(height, width);
            }
            if(depthimage.at<u_int16_t>(height, width)<min_distance)
            {
              min_distance = depthimage.at<u_int16_t>(height, width);
            }
          }
      ...
      distance = total_distance/num_depth_points/1000;
      ...
      dist_val[4] = depthimage.at<float>(result.y+result.height/2 , result.x+result.width/2) ;
        ...
        dist_val[0]=dist_val[0]/255;
        dist_val[1]=dist_val[1]/255;
        dist_val[2]=dist_val[2]/255;
        dist_val[3]=dist_val[3]/255;
        dist_val[4]=dist_val[4]/255;

      /*ROS_INFO("dist_val[0]:%d",dist_val[0]);
      ROS_INFO("dist_val[1]:%d",dist_val[1]);
      ROS_INFO("dist_val[2]:%d",dist_val[2]);
      ROS_INFO("dist_val[3]:%d",dist_val[3]);
      ROS_INFO("dist_val[4]:%d",dist_val[4]);
      ...
      std::cout << "num_depth_points:" << num_depth_points << std::endl;
      //ROS_INFO("distance:%d",distance);
      //ROS_INFO("num_depth_points:%d",num_depth_points);
