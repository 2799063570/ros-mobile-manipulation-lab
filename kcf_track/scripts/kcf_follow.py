...
class Follower:
	def __init__(self):
		...
		self.positionSubscriber = rospy.Subscriber(
			'/kcf/track', Twist, self.positionUpdateCallback)
	...
	def positionUpdateCallback(self, twist):
		...
		linearSpeed = max(-self.line_max_speed, min(linearSpeed, self.line_max_speed))
		angularSpeed = max(-self.angular_max_speed, min(angularSpeed, self.angular_max_speed))
		if angleX == 0:
			angularSpeed = 0.0
		if distance == 0:
			linearSpeed = 0.0
