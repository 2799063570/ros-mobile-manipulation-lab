...
***********************************/
	...
	else if(str1 == str17 && if_six == "no" && sw == "on")
	{
		sw = "off";
		system("aplay -D plughw:CARD=Device,DEV=0 ~/wheeltec_robot/src/xf_mic_asr_offline/feedback_voice/OK.wav");
	...
	else if(str1 == str19 && if_six == "no" && sw == "off")
	{
		system("aplay -D plughw:CARD=Device,DEV=0 ~/wheeltec_robot/src/xf_mic_asr_offline/feedback_voice/OK.wav");
		cout<<"好的：关闭视觉跟随"<<endl;
		sw = "on";
...
int main(int argc, char** argv)
{
	...
	ros::init(argc, argv, "command_recognition");     //初始化ROS节点
  ...
  /***创建自主导航目标点话题发布者***/
	...
	n.param<std::string>("/if_six", if_six, std::string("no"));
	n.param<std::string>("/if_akm_yes_or_no", if_akm, std::string("no"));
  ...
  /***用户界面***/
	...
	if(if_six == "no")
	{
		...
		cout<<"小车视觉跟随———————————>小车打开视觉跟随"<<endl;
		cout<<"关闭视觉跟随———————————>小车关闭视觉跟随"<<endl;
	...
	// while(ros::ok())
	// {
	//   ros::V_string car_nodes;
 //      ros::master::getNodes(car_nodes);
 //      std::string node_name = std::string("/joint_state_publisher");
 //      auto if_publish = std::find(car_nodes.begin(),car_nodes.end(),node_name.c_str());
 //       if(if_publish == car_nodes.end()){
 //         system("dbus-launch gnome-terminal -- roslaunch wheeltec_arm_six voi_demo.launch");
 //         sleep(3);
