...
本方案保留原腕部 RGB 相机。`workspace_camera` RGB-D 相机仅作为特定任务场景的
可选外部相机，不安装在通用机器人模型或车体立柱上。两类相机职责分离：

- `workspace_camera`：全局目标检测、三维定位、MoveIt OctoMap；
- `hand_camera`：近距离观察以及后续眼在手上精定位。

场景若需要 `workspace_camera`，必须自行提供安装结构及其到机器人基准坐标系的
标定变换；不需要时不要启动本方案中的 RGB-D、点云或 OctoMap 节点。
...
真实相机入口默认使用 RealSense 风格话题。当前配置关闭相机驱动自身 TF，因此具体
任务场景必须在自己的 URDF/Xacro 中发布完整相机坐标树：
...
相机内参、场景基准坐标系到 `workspace_camera_link` 的外参、点云方向、OctoMap和
RViz轨迹检查之前，不得切换到真机执行模式。

场景 Xacro 中的安装位置和俯角只能作为结构初值，不能代替标定结果。真机必须用
实际测量或手眼标定结果更新该场景中 `workspace_camera` 的 `xyz/rpy`。

可复用相机宏对 RealSense 深度和彩色光学坐标系之间的内部偏移采用近似值，适合模型
联调，不适合作为高精度抓取外参。真机推荐只在场景 URDF 中保存外部安装标定结果，
由 RealSense 驱动发布设备
