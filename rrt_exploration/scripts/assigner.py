...
def node():
    ...
    # This can be smaller than the laser scanner range. A smaller value reduces
    # computation time, but a value that is too small makes information gain inaccurate.
    ...
    while not rospy.is_shutdown():
        ...
        # -------------------------------------------------------------------------
        # Get discount and update informationGain
        for i in nb + na:
            ...
            for ir in nb:
                for ip in range(0, len(centroids)):
                    ...
                    if ((norm(centroids[ip] - robots[ir].assigned_point)) < hysteresis_radius):
                        information_gain = informationGain(mapData,
