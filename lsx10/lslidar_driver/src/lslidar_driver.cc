...
namespace lslidar_driver
{
	...
	bool LslidarDriver::initialize() // 根据连接方法初始化
	{
		...
		pu_ = nh.advertise<std_msgs::Int8>("lidar_status", 3);
		std_msgs::Int8 msg_status;
		msg_status.data = 0;
		pu_.publish(msg_status);
	...
	void LslidarDriver::data_processing(unsigned char *packet_bytes, int len) // 处理每一包的数据
	{
		...
		{
			int s_e = packet_bytes[rpm_bits_start];
			int z_e = packet_bytes[rpm_bits_start + 1];
			if (s_e * 256 + z_e == 0)
				z_e++;
			int rpm = 2500000 / (s_e * 256 + z_e);
			if (rpm < 150 || rpm > 1500)
			{
				rpm_err++;
				if (rpm_err > 10)
				{
					ROS_ERROR("Lidar rotation speed is abnormal !\n");
					std_msgs::Int8 msg_status;
					msg_status.data = -3;
					pu_.publish(msg_status);
				}
		...
		if (zero_num > 1500)
		{
			ROS_ERROR("All laser data are zero!\n");
			std_msgs::Int8 msg_status;
			msg_status.data = -1;
			pu_.publish(msg_status);
		}
	...
	void LslidarDriver::data_processing_3(unsigned char *packet_bytes, int len) // 处理每一包的数据/N301_1.8
	{
		double degree = (double)((packet_bytes[20] * 16777216 + packet_bytes[21] * 65536 + packet_bytes[22] * 256 + packet_bytes[23]) / 10000.0);
		double degree_interval = (double)((packet_bytes[24] * 16777216 + packet_bytes[25] * 65536 + packet_bytes[26] * 256 + packet_bytes[27]) / 10000.0);

		for (int num = 0; num < 250; num++)
		{
			int s = packet_bytes[num * 3 + 28];
			int z = packet_bytes[num * 3 + 29];
			scan_points_[idx].range = double((z * 256 + (s)) * 0.004);
			scan_points_[idx].intensity = int(packet_bytes[num * 3 + 30]);
			scan_points_[idx].degree = double(degree + degree_interval * num);
			// printf("degree[%d] = %f range = %f\n",idx,scan_points_[idx].degree,scan_points_[idx].range);
			idx++;
		}

		if (packet_bytes[11] == 0x04)
		{
			count_num = idx;
			idx = 0;
			for (int k = 0; k < count_num; k++)
		...
		while (ros::ok())
		{
			...
			if (lidar_name == "N10_P" || lidar_name == "M10_DOUBLE")
			{
				if (pubScan)
				{
					...
					msg.angle_increment = 2 * M_PI / (double)(count_num-1);
				...
				if (pubPointCloud2)
				{
					...
					for (uint16_t i = 0; i < count_num; i++)
					{
						...
						if (!pass_point)
						{
							...
							point.timestamp = timestamp - scan_time + point_idx * (scan_time / count_num);
	...
	bool LslidarDriver::polling() // 主循环函数
	{
		...
		if (interface_selection == "net")
		{
			...
			while (true)
			{
				...
				if (lidar_name == "N301_1_8" && (packet->data[0] == 0xA5 && packet->data[1] == 0xFF && packet->data[2] == 0x00 && packet->data[3] == 0x5A))
						len = 1206;
					
				if ((len <= 0 || len > 1206 || packet->data[0] != 0xa5 || packet->data[1] != 0x5a) && lidar_name != "N301_1_8")
					continue;
				if ((packet->data[0] != 0xa2 || packet->data[1] != 0x5c) && lidar_name == "N301_1_8")
					continue;
				for (int i = 0; i < len; i++)
				{
		...
		{
			if (lidar_name == "N301_1_8")
				LslidarDriver::data_processing_3(packet_bytes, len);
			else if (lidar_name == "N10_P" || lidar_name == "M10_DOUBLE")
				LslidarDriver::data_processing_2(packet_bytes, len);
			else
				LslidarDriver::data_processing(packet_bytes, len);
		}
